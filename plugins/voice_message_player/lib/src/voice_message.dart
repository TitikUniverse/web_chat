...
class _VoiceMessageState extends State<VoiceMessage>
  ...
  @override
  Widget build(BuildContext context) => _rowChild(context);
    ...
    return Theme(
      ...
      child: SizedBox(
        ...
        child: Stack(
          ...
          children: [
            widget.me ? const Noises(noiseColor: Color.fromARGB(255, 207, 166, 107)) : const ContactNoise(noiseColor: Color.fromARGB(255, 207, 166, 107)),
  ...
  void _startPlaying() async {
    if (widget.audioFile != null) {
      ...
      Uint8List dataBuffer = (await widget.audioFile!).readAsBytesSync();
      debugPrint("> _startPlaying path $path");
      await _mPlayer!.startPlayer(fromDataBuffer: dataBuffer);
      // await _player.play(DeviceFileSource(path));
    } else if (widget.audioSrc != null) {
      _mPlayer!.startPlayer(fromURI: widget.audioSrc!);
      // await _player.play(UrlSource(widget.audioSrc!), ctx: AudioContext(iOS: AudioContextIOS(category: AVAudioSessionCategory.playback, options: [AVAudioSessionOptions.mixWithOthers, AVAudioSessionOptions.defaultToSpeaker])));
    }
    _controller!.forward();
  }
  ...
  void _setDuration() async {
    if (widget.duration != null) {
      ...
      _audioDuration = await jsAudio.AudioPlayer().setUrl(widget.audioSrc!, preload: true);
    }
    duration = _audioDuration!.inMilliseconds;
